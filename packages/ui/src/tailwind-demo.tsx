...
const features = ['Shared Tailwind utilities', 'Shadcn-style primitives', 'Rendered from @repo/ui']
    ...
    <Card className="w-full max-w-5xl overflow-hidden bg-[linear-gradient(135deg,rgba(79,124,255,0.18),rgba(15,23,42,0.92))]">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <CardDescription className="max-w-2xl text-slate-200">
              ...
              <code className="rounded bg-white/10 px-2 py-1 text-slate-100">packages/ui</code> and
              now uses reusable Button, Card, Input, Label, and Separator components.
            </CardDescription>
          </div>
        </div>
      ...
      <CardContent className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        ...
        <div className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5 text-sm leading-6 text-slate-200">
          <p>
            The same primitives can now be imported from the shared package and reused in both
            sub-apps without adding separate component copies.
          </p>
        </div>
      </CardContent>
